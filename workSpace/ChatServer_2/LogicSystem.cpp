...
void LogicSystem::DealMsg() {
	for (;;) {
		...
		///找到：调用对应业务处理函数
		///，如 LoginHandler / HeartBeatHandler / DealChatTextMsg 等，传入
...
void LogicSystem::AuthFriendApply(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data) {
	...
	auto touid = root["touid"].asInt(); // 是不是当前chatServer用户的uid
	...
	std::string base_key = USER_BASE_INFO + std::to_string(touid); //获取被添加人的信息
