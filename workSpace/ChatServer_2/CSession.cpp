...
void CSession::AsyncReadBody(int total_len)
{
	...
	///精确读取消息体长度（粘包/拆包抽象）total_len --> 消息体的长度。
	asyncReadFull(total_len, [self, this, total_len](const boost::system::error_code& ec, std::size_t bytes_transfered) {
		try {
			...
			///将data数据拷贝到消息体结构中 ,长度为bytes_transfered---
			///之前已经插入了msgid , msglen
...
void CSession::AsyncReadHead(int total_len)
{
	...
	asyncReadFull(HEAD_TOTAL_LEN, [self, this](const boost::system::error_code& ec, std::size_t bytes_transfered) {
		try {
			...
			///_recv_head_node 只承载“当前消息头
			_recv_head_node->Clear();
			///将data数据拷贝到头部结构中 ,长度为bytes_transfered
			/// 这是头部的数据
			...
			/// 将消息体结构初始化 加入id和长度
			///_recv_msg_node 承载“当前消息体”并且加入了 msglen ,msgid;
...
//读取完整长度
///封装函数：目标是“异步读取恰好 maxLength 字节后调用回调 handler”。
///它本身不直接发起 socket 读取，而是做初始化后调用递归函数 asyncReadLen。
